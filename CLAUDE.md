...
A separate **`make type-check`** gate runs `ty` plus `tools/check_local_annotations.py --include-module` — an AST checker enforcing that **every local AND module-level (global) variable across `src`, `tests`, and `ports/**` carries an explicit annotation** (not just an inferred type). It exempts what can't carry an inline annotation: tuple-unpack, `for`/`with`/`except`/comprehension/walrus targets, augmented assignment, attribute/subscript targets, class-body assignments (Enum members / class attrs), names declared `global`/`nonlocal` in a function, and `TypeVar`/`ParamSpec`/`TypeVarTuple` definitions. The checker lives only in `make type-check`, not `make format`. (`ty`'s own scope: src, tests, ports/codetheclassics; `ports/openglsuperbiblev4` is annotated but kept out of the `ty` step — it has pre-existing glfw/imgui stub diagnostics unrelated to annotations.)
...
CI (`.github/workflows/`) is a **thin wrapper over the make/Dockerfile system**: every workflow is `checkout` → `make <target>`, with all build/check/release logic in make targets so it runs identically in CI and on a laptop (`CONTAINER_CMD` auto-detects podman→docker; runners are ubuntu-latest, which ships Docker). If CI needs a step, add it as a `make` target first (runnable locally), never as inline YAML. `checks.yml` runs on every push + PR as two check-only jobs — `format` (`make check-format`: runs the formatter in the disposable runner, then fails on a `git diff`) and `type` (`make type-check`: ty + the local/module annotation checker). Neither commits or reformats the repo. `release.yml` (on a `v*` tag) = `make release-tarball` + `make image-push` (source + the three book forms → a GitHub Release; image → ghcr.io) — cut a release with `make release` (tags `v$(VERSION)` from pyproject; push it to trigger). The pushed image **bakes the source** so it's pullable-and-runnable standalone while `make shell` overrides it via the live mount (`tasks/reference/container-source-and-mounts.md`). Rationale/roadmap: `tasks/github-actions-format-ci.md`, `tasks/github-actions-release-ci.md`.
