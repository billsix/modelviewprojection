...
def draw_triangles(vao: int, vertex_count: int, time: float) -> None:
    ...
    GL.glUniform1f(triangle_pipeline.u_fov, PROJ_FOV)
    GL.glUniform1f(triangle_pipeline.u_aspect, PROJ_ASPECT)
...
def _emit_axis(r: float, g: float, b: float, grayed_out: bool) -> None:
    ...
    GL.glUniform1f(axis_pipeline.u_fov, PROJ_FOV)
    GL.glUniform1f(axis_pipeline.u_aspect, PROJ_ASPECT)
