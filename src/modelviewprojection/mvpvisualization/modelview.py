...
"""modelview, on the Cayley-graph engine.

A FULL CONVERT of the original (which was fixed-function immediate mode) to the
3.3-core shader toolkit + the Cayley engine: object placement, the camera placed
as an object, and the world->camera ``InverseOperations`` (the original's
...
def graph_panel(t: float) -> None:
    ...
    imgui.set_next_window_size(
        imgui.ImVec2(460, 480), imgui.Cond_.first_use_ever
    )
