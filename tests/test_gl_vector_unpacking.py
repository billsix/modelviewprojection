...
# g2.Vector/g3.Vector iterating their coordinates in (e_1, e_2[, e_3]) order
# with exactly the right arity.  If a gacalc upgrade ever changes that
# order/arity, these fail loudly here instead of the demos silently drawing
# garbage.  Pure data -- no display / no real GL needed.
...
def test_vector3_iterates_as_x_then_y_then_z() -> None:
    v: g3.Vector = g3.Vector(1.0, 2.0, 5.0)
...
def test_unpacking_matches_glvertex_positional_args() -> None:
    ...
    assert record_positional_args(*g2.Vector(7.0, 8.0)) == (7.0, 8.0)
    assert record_positional_args(*g3.Vector(7.0, 8.0, 9.0)) == (7.0, 8.0, 9.0)
