...
- **The "cross product" is a gacalc wedge.** `is_counter_clockwise` is literally `float((v1 ^ v2).coeff_e_12) >= 0.0` (the `.coefficient(Bivector2.e_12)` spelling was replaced when gacalc 0.0.12 gave the wedge a precise `Bivector2` return type — `tasks/archive/2026/07/22/dual-coefficient-cleanup.md`). In 2D the bivector coefficient of `v1 ^ v2` *is* the signed area / 2D cross product / `|v1||v2|sin θ`. The code uses it **unnormalized on purpose** — dividing by magnitudes wouldn't change the sign but would blow up when a vector is zero (a pixel sitting exactly on a triangle vertex). This is the same wedge-as-signed-area idea as `mathutils.sine` (§3).
...
**Distinct from Code the Classics:** the SuperBible ports are *translations of a third-party book's C++*; the pgzero ports are *translations of third-party games' Python*. Neither adopts the Cayley-graph idiom — they're faithful to their sources, which is the whole point of keeping them under `ports/`. **One nuance:** 13 SuperBible ports (the lit/shadow family — Block, litjet, shadow, shinyjet, the sphereworlds, pyramid, fogged, multisample, SphereWorld32) do use mvp's `mathutils.find_normal`/`plane_equation`, and 12 of them import `Vector3` *through* `modelviewprojection.mathutils` — the removed-facade anti-pattern surviving by accident (it works only because `mathutils` happens to import `Vector3` internally; `__all__` doesn't gate `from X import Y`). Known deviation — new code imports from `gacalc.g3` directly; don't "fix" the 12 unasked, but don't copy them. (Some demo names — Block, sphereworld, atom, solar — appear in *both* the SuperBible ports and Bill's curriculum demos, with different intent; don't conflate them.) Full porting rulebook + chapter/stub inventory: **`tasks/reference/superbible-ports-guide.md`**.
