...
**How the shim works (`pgzero_gl/`) — the PACKAGE source, which since 2026-09-05 no game imports.** Steps 1–2 of `tasks/pgzero-gl-inline-strip-reextract.md` inlined it into each game and the tightening pass (`tasks/reference/code-the-classics-tightening.md`) reshaped each copy; the package copy stays, unused, until step 3 (parked) decides its fate. What follows describes the design, which the inlined engines inherit:
...
**Headless verification.** The old `_smoketest.py` (one EGL-pbuffer frame per game, importing the game as a module) was **removed 2026-09-06**: no game has been importable since the inlining pass made each own its loop, and the tightened games exit on import. The gates are `tools/ctc_verify_game.sh` (frame-N pixel identity vs a git ref, under Xvfb, in the nested image) and `tools/ctc_state_trace.py` + `tools/ctc_compare_traces.py` (scripted-input state trace) — `tasks/reference/tests-and-gates.md`. Env vars the games still honour: `PGZERO_GL_INFO=1`, `PGZERO_MAX_FRAMES=N`. (History: `tasks/archive/2026/07/25/fix-smoketest-broken-pgzrun.md`, `tasks/archive/2026/09/06/remove-ctc-smoketest.md`)
