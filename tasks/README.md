...
| Task | Status | Doc |
|------|--------|-----|
| **Migrate mvp math onto `gacalc`** (delete overlapping vector/transform math, import gacalc vectors; rebuild graphics-only math on top) | proposed — needs go-ahead; blocks on a gacalc PyPI release (animation layer) + book-strategy decision | [`gacalc-math-migration.md`](gacalc-math-migration.md) |
