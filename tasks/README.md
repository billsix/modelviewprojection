...
| **Move keyboard render-options to imgui** (mode toggles/shader-select/etc. → checkboxes/combos) | implemented — all 18 demos converted + structurally verified; pending Bill's visual check + commit | [`ports-render-options-to-imgui.md`](ports-render-options-to-imgui.md) |
| **Mirror every keyboard control in an imgui menubar** (94 demos) | ✅ complete 2026-05-29 — all 94 demos have a top menubar; imgui-leftover-texture-state fallout swept (multitexture/cubemap/texgen/pixbufobj fixed, rest safe, vertexshaders confirmed fine) | [`ports-mirror-keyboard-in-imgui.md`](ports-mirror-keyboard-in-imgui.md) |
| **hdrbloom crashes on startup** (after-glow PBO allocated 1 byte) | ✅ fixed 2026-05-29 (sized the PBO in setup_rc); pending Bill's run-to-confirm | [`hdrbloom-pbo-sizing-crash.md`](hdrbloom-pbo-sizing-crash.md) |
