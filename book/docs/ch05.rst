...
We added a top-level ``translate`` function.  Given a translation amount ``b``,
``translate(b=b)`` returns an invertible function that shifts any vector by ``b``
...
.. code:: Python
       ...
       >>> from modelviewprojection.mathutils import Vector2, translate
...
We will define each vertex relative to the paddle's center using two special
unit vectors, the *natural basis*: ``e_1`` is one unit along the x axis, and
``e_2`` is one unit along the y axis.  ``Vector2`` provides them (and the
origin) as the class constants ``Vector2.e_1``, ``Vector2.e_2``, and
``Vector2.zero()``.

Any 2D vector is a *scaled sum* of these two: scale each basis vector by a
