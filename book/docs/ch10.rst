...
following the direction of each edge.  In our new Cayley graph, when going from world
...
The inverse of ``translate(b=b)`` is ``translate(b=-b)`` -- shift back by the
negative.  The built-in ``inverse`` agrees:

.. code:: Python

      >>> from modelviewprojection.mathutils import Vector2, translate, inverse
      >>> e_1, e_2 = Vector2.e_1(), Vector2.e_2()
      >>> b = 3.0 * e_1 + 4.0 * e_2
      >>> p = 10.0 * e_1 + 20.0 * e_2
      >>> inverse(translate(b=b))(p).isclose(translate(b=-b)(p))
      True
...
The inverse of ``rotate(angle)`` is ``rotate(-angle)`` -- rotate back by the
negative angle.  The built-in ``inverse`` agrees:

.. code:: Python

      >>> import math
      >>> from modelviewprojection.mathutils import Vector2, rotate, inverse
      >>> e_1, e_2 = Vector2.e_1(), Vector2.e_2()
      >>> angle = math.radians(90.0)
      >>> p = 1.0 * e_1 + 2.0 * e_2
      >>> inverse(rotate(angle))(p).isclose(rotate(-angle)(p))
      True

Inverse Of Scale
&&&&&&&&&&&&&&&&

The inverse of ``scale_non_uniform_2d(m_x, m_y)`` is
``scale_non_uniform_2d(1.0 / m_x, 1.0 / m_y)`` -- scale by the reciprocals.
The built-in ``inverse`` agrees:

.. code:: Python

      >>> from modelviewprojection.mathutils import Vector2, scale_non_uniform_2d, inverse
      >>> e_1, e_2 = Vector2.e_1(), Vector2.e_2()
      >>> p = 8.0 * e_1 + 12.0 * e_2
      >>> inverse(scale_non_uniform_2d(2.0, 4.0))(p).isclose(
      ...     scale_non_uniform_2d(1.0 / 2.0, 1.0 / 4.0)(p))
      True
...
The inverse of ``compose([f1, f2])`` is ``compose([inverse(f2), inverse(f1)])``
...
.. code:: Python
      ...
      ...     Vector2, translate, rotate, compose, inverse,
      ... )
      >>> e_1, e_2 = Vector2.e_1(), Vector2.e_2()
...
Run "python src/modelviewprojection/mvpvisualization/modelviewperspectiveprojection.py", and follow along with the Cayley
...
The attentive reader may notice: "Bill, you said the inverse causes the operations
