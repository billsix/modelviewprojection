...
Replace lambda stacks with OpenGL 2.1 :term:`matrix stacks <Matrix Stack>`, provided
by the driver for your graphics card.  This is how preshader
opengl worked.

The :term:`function stack <Function Stack>` allowed us to aggregate the entirety of
the transformations from :term:`modelspace<Modelspace>` to :term:`NDC <Normalized Device Coordinates>`, by creating a context
...
given one matrix, it can perform a sequence of :term:`transformations <Transformation>`
from one space to another, with one matrix multiplication.

OpenGL 2.1 deals with two different types of matrices: 1) the :term:`projection
matrix <Projection Matrix>`, which effectively is the function from :term:`camera space <Camera Space>`
to NDC (:term:`clip space <Clip Space>`) and 2) The :term:`model-view matrix <Model-View Matrix>`, which deals
...
across multiple spaces in the :term:`Cayley graph <Cayley Graph>`, it may appear
...
transformation sequence.  For instance, if we are at :term:`world space <World Space>`,
the Model-View matrix at the top of the :term:`matrix stack <Matrix Stack>` will convert
...
we can save onto the current frame of reference with a "glPushMatrix",
and restore the saved state by "glPopMatrix".

Use glPushMatrix and glPopMatrix to save/restore a local :term:`coordinate
system <Coordinate System>`, that way a tree of objects can be drawn without one child
destroying the relative coordinate system of the parent node.

In src/modelviewprojection/mvpvisualization/pushmatrix.py, the grayed out
...
Load src/modelviewprojection/demos/demo19.py in Spyder and hit the play button.
...
First thing to note is that we are now using OpenGL 2.1's official :term:`transformation <Transformation>`
procedures, and in the projection transformation, they flip the z axis, making
it a left hand :term:`coordinate system <Coordinate System>`.  The reason for this is long, and I
have begun discusses in the "Standard Perspective Matrix" section, but it is an
incomplete section for now.
But for now, all it means that we have to change how the :term:`depth test <Depth Test>`
will be configured, as after the projection transformation, the far z
value is 1.0, and the near z value is -1

The clear depth that is set for each :term:`fragment <Fragment>` each :term:`frame <Frame>` is now 1.0,
...
.. literalinclude:: ../../src/modelviewprojection/demos/demo19.py
   :language: python
   ...
   :linenos:
   :lineno-match:
   :caption: src/modelviewprojection/demos/demo19.py
...
Set the model, view, and :term:`projection matrix <Projection Matrix>` to the :term:`identity <Identity>`
...
change the projection matrix to convert the :term:`frustum <Frustum>`
to :term:`clip space <Clip Space>`
set the projection matrix to be perspective.  Since
the :term:`viewport <Viewport>` is always square, set the :term:`aspect ratio <Aspect Ratio>` to be 1.0.
We are now going to clip space instead of to :term:`NDC <Normalized Device Coordinates>`, which we be discussed
in the next chapter.
...
"glMatrixMode" tells the computer which :term:`matrix stack <Matrix Stack>` should be the active one,
...
and we will ignore the implementation of it; it is now a :term:`black box <Black Box vs White Box>`.

Now onto :term:`camera space <Camera Space>`!

The camera's position could be described relative to :term:`world space <World Space>` by
...
do the :term:`inverse <Inverse>` operations to the view stack.
...
First thing we did was set the current matrix to be the :term:`model-view matrix <Model-View Matrix>`, instead
of the projection matrix.  Most of our work will be with the model-view matrix,
as looking at the :term:`Cayley graph <Cayley Graph>`, there's only one function from camera space
...
with OpenGL matrices, each transformation, such as :term:`translate <Translation>`, :term:`rotate <Rotation>`, and :term:`scale <Scaling>`,
...
but since we loaded the :term:`projection matrix <Projection Matrix>` and the :term:`modelview
matrix <Model-View Matrix>` into OpenGL, glVertex3f will apply those transformations for us [1]_!
...
we are drawing, and upon the next iteration of the :term:`event loop <Event Loop>`,
all 3 matrices will be reset to the :term:`identity <Identity>`
...
.. [1] To note.  The computer has no notion of these relative coordinate systems.
       From the computer's point of view, there is one :term:`coordinate system <Coordinate System>`, (x,y,z,w).
       ...
       "glVertex", given :term:`modelspace<Modelspace>` data, must pull 2 matrices from OpenGL's :term:`matrix stacks <Matrix Stack>`,
       the modelview and the projection.  It then does two :term:`transformations <Transformation>` to turn
       the :term:`modelspace<Modelspace>` data in :term:`NDC <Normalized Device Coordinates>` (:term:`clip-space <Clip Space>`, where the x y and z are each divided by the w to get
       ...
       be a subject for after the midterm, but the 4 :term:`vertices <Vertex>` will be converted to :term:`screen space <Screen Space>`,
       given the matrices, and when "glEnd" is called, the graphics driver will need to determine
       what :term:`pixels <Pixel>` are within the quadrilateral or not.  For the :term:`fragments <Fragment>` within the quadrilateral,
