...
When running Demo02, if the user resizes the :term:`windows <Window>`,
then the paddles lose their proportionality, as NDC
no longer is mapped to a square :term:`screen-space <Screen Space>`.
...
Load src/modelviewprojection/demos/demo03.py in Spyder and hit the play button.
...
OpenGL has a solution for us.  The :term:`viewport <Viewport>` is a rectangular region contained
...
The setup code is the same as the previous demo's setup.  Initialize :term:`GLFW`.  Set the OpenGL version.
...
.. literalinclude:: ../../src/modelviewprojection/demos/demo03.py
   :language: python
   :start-after: doc-region-begin square viewport
   :end-before: doc-region-end square viewport
   :linenos:
   :lineno-match:
   :caption: src/modelviewprojection/demos/demo03.py


* declare a function to configure OpenGL to draw only in a square subset of the :term:`monitor <Monitor>`, i.e. the viewport
...
* glClear clear the color of every :term:`pixel <Pixel>` in the whole :term:`frame buffer <Frame Buffer>`, independent of viewport.
...
* To make a square sub-region, we need a number for the distance between :term:`vertices <Vertex>` of the square.
...
* Enable the :term:`scissor test <Scissor Test>`.  Internally, OpenGL drivers likely have global variables
...
* the scissor test allows us to specify a region
  ...
  because of the existing call to glClearColor.
  By calling glScissor, we are setting a value in each :term:`fragment <Fragment>` (i.e., pixel) on a square
...
* Look at the image above of NDC superimposed on :term:`Screen Space`.  From this,
...
will be mapped to the region of screen coordinates
...
This demo's :term:`event loop <Event Loop>` is just like the previous demo,
...
* The event loop is the same as the previous demo, except
  that we call draw_in_square_viewport every :term:`frame <Frame>` at the beginning.
