...
Fix the :term:`rotation <Rotation>` problem from the previous demo in a seemingly intuitive
...
Load src/modelviewprojection/demos/demo08.py in Spyder and hit the play button.
...
The problem in the last demo is that all :term:`rotations <Rotation>` happen relative
to :term:`World Space`'s (0,0) and axes.  By translating our paddles to their position
before rotating, they are rotated around World Space's :term:`origin <Origin>`, instead
of being rotated around their :term:`modelspace<Modelspace>`'s center.


In this demo, we try to solve the problem by making a function to rotate
...
.. literalinclude:: ../../src/modelviewprojection/mathutils.py
   ...
   :end-before: doc-region-end rotate around signature
   :language: python
   :linenos:
   :lineno-match:
...
.. literalinclude:: ../../src/modelviewprojection/mathutils.py
   :start-after: doc-region-begin rotate around body
...
Within the :term:`event loop <Event Loop>`, this seems quite reasonable




.. literalinclude:: ../../src/modelviewprojection/demos/demo08.py
   :language: python
   :start-after: doc-region-begin begin event loop
   :end-before: doc-region-end begin event loop
   :linenos:
   :lineno-match:
   :caption: src/modelviewprojection/demos/demo08.py
...
All we did was add a rotate around function, and call it, with the
...
not thought out well at all.  We apply a :term:`transformation <Transformation>`
from paddle space to world space, then do the :term:`inverse <Inverse>`, then rotate,
...
and the :term:`Cayley graph <Cayley Graph>` is gross.
...
resetting the :term:`coordinate system <Coordinate System>`
