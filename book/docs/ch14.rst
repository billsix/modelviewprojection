...
The natural basis extends to 3D with a third unit vector, ``e_3``, one unit
along the z axis --- the class constants ``Vector3.e_1``, ``Vector3.e_2``, and
``Vector3.e_3``.

A 3D vertex is then ``a * e_1 + b * e_2 + c * e_3`` --- the same scaled-sum form
...
do a sequence of transformations to the :term:`modelspace<Modelspace>` data to get
