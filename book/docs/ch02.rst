...
Learn how to draw rectangles in an OpenGL window.  Learn about OpenGL's :term:`coordinate system <Coordinate System>`, :term:`Normalized Device Coordinates` (NDC),
which are from -1.0 to 1.0, in the X, Y, and Z directions.  Geometry drawn outside
of this region will not be displayed in the :term:`window <Window>`.
...
Load src/modelviewprojection/demos/demo02.py in Spyder and hit the play button.
...
The setup code is the same.  Initialize :term:`GLFW`.  Set the OpenGL version.
...
Within the :term:`event loop <Event Loop>`, demo02.py draws 2 rectangles, as one might see in
a game of Pong.

.. literalinclude:: ../../src/modelviewprojection/demos/demo02.py
   :language: python
   :start-after: doc-region-begin event loop
   :end-before: doc-region-end event loop
   :linenos:
   :lineno-match:
   :caption: src/modelviewprojection/demos/demo02.py
...
"glColor3f" is a function that sets a global variable, which makes
it the color to be used
for the subsequently-drawn graphical shape(s).  Let's make the first
paddle purple, and a second paddle yellow.

To specify the corners of the rectangle,
"glBegin(GL_QUADS)" tells OpenGL that we will soon specify 4 :term:`vertices <Vertex>`,
...
* done with :term:`frame <Frame>`, flush the current buffer to the monitor
* Swap front and back buffers



The frame sent to the :term:`monitor <Monitor>` is a set of values like this ::
...
into :term:`pixel <Pixel>` coordinates (i.e. pixel (10,15))?  Nothing, OpenGL does that for us automatically; therefore
...
had to explicitly set the color of individual pixels using the pixel's coordinates, the programmer would have to
program using "screen-space" (Any space_ means a system of numbers which you're using.
:term:`Screen-space <Screen Space>` means you're specifically using pixel coordinates, i.e, set pixel (5,10) to be yellow).
...
:term:`aspect ratio <Aspect Ratio>`, for now).
...
a :term:`Cayley Graph` [1]_ :cite:`AbstractAlgebra`, of the two spaces
shown so far.  The nodes represent a :term:`coordinate system <Coordinate System>` (i.e. :term:`origin <Origin>` and axes),
and the directed edge represents an :term:`invertible function <Invertible Function>` that converts coordinates from one
...
if you label each :term:`vector <Vector (Vector2 / Vector3)>` on all three graphs, and look at the axes to find
...
.. [1] I rediscovered Cayley Graphs when looking at Figure 1.3 of
       ...
       the camera in :term:`world space <World Space>` is the
       same process of placing any object in world space, so the arrow in
       :cite:`MathematicsFor3DGame` from World Space to :term:`Camera space <Camera Space>` could be
       reversed, to represent the placement of the camera in World Space.
       It was two years
       later that I was reading :cite:`AbstractAlgebra` and I realized
...
functions that :term:`change basis <Change of Basis>`, i.e. coordinate conversion.
...
The Wikipedia article for :term:`Cayley graphs <Cayley Graph>` (https://en.wikipedia.org/wiki/Cayley_graph) is intimidating,
...
An univariate example use of :term:`Cayley graphs <Cayley Graph>` is exchanging coins with
a bank.
...
the function, i.e. :term:`transformation <Transformation>`, applies.
...
of any directed edge that you traverse against its direction.
...
:term:`Function composition <Function Composition>` is used extensively throughout this book.  As a reminder
...
omits the argument :inlinetex:`x`
...
Another univariate example use of :term:`Cayley graphs <Cayley Graph>` is converting a temperature
...
to figure out what to ignore. To quote Gerald Sussman in the SICP video lectures
...
Below is a :term:`Cayley graph <Cayley Graph>` that shows
:term:`coordinate systems <Coordinate System>` "a", "b", "c", and "d", and functions are provided
