...
A definition of this function is not provided right now, because we can't change it in software, but we must recognize that it exists.
...
To convert those 100 penny into quarters, on the Cayley graph, move from
...
Another uni-variate example use of Cayley graphs is converting a temperture
...
to figure out what to ignore.  Below is a Cayley graph that shows
...
To make it more concrete, using the temperature example, the following
is python code to create functions to convert between fahrenheit, celcius, and kelvin

.. literalinclude:: ../../tests/test_mathutils1d.py
   :language: python
   :start-after: doc-region-begin temperature functions
   :end-before: doc-region-end temperature functions
   :linenos:
   :lineno-match:
   :caption: tests/test_mathutils1d.py
