...
Load src/modelviewprojection/demos/demo11.py in Spyder and hit the play button.
...
Define the geometry of the square in its own :term:`modelspace <Modelspace>`.
...
.. literalinclude:: ../../src/modelviewprojection/demos/demo11.py
   ...
   :linenos:
   :lineno-match:
   :caption: src/modelviewprojection/demos/demo11.py
...
As a refresher, the author recommends reading the code from :term:`modelspace <Modelspace>`
to :term:`worldspace <World Space>` from the bottom up, and from worldspace to :term:`NDC <Normalized Device Coordinates>` from top down.

* Read from :term:`modelspace <Modelspace>` to :term:`world space <World Space>`, bottom up
* Reset the :term:`coordinate system <Coordinate System>`
* Read from :term:`world space <World Space>` to :term:`camera space <Camera Space>`, knowing that camera transformations
  are implemented as the :term:`inverse <Inverse>` of placing the camera space in world space.
* Reset the :term:`coordinate system <Coordinate System>`
* Read :term:`camera-space <Camera Space>` to :term:`NDC <Normalized Device Coordinates>`

New part!  Draw the square relative to the first paddle!
Translate the square to the right by 2 units.
We are dealing with a -1 to 1 :term:`world space <World Space>`, which
later gets scaled down to :term:`NDC <Normalized Device Coordinates>`.
...
Towards that, we need to do all of the :term:`transformations <Transformation>` to the square
that we would to the paddle, and then do any extra transformations
afterwards.

As such, read

* Read paddle1space to :term:`world space <World Space>`, from bottom up
...
* Read :term:`modelspace <Modelspace>` to paddle1space, from bottom up
* Reset the :term:`coordinate system <Coordinate System>`.

Now the square's geometry will be in its own space!

* Read from :term:`worldspace <World Space>` to :term:`camera-space <Camera Space>`, knowing that camera transformations
  are implemented as the :term:`inverse <Inverse>` of placing the camera space in world space.
