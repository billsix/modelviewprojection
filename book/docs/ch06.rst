...
Load src/modelviewprojection/demos/demo06.py in Spyder and hit the play button.
...
Whatever a convenient numbering system is (i.e. :term:`coordinate system <Coordinate System>`) for modeling objects
is called :term:`modelspace<Modelspace>`.  Since a paddle has four corners, which corner should be at
the :term:`origin <Origin>` (0,0)?  If you don't already know what you want at the origin, then
...
:term:`scaling <Scaling>` and :term:`rotating <Rotation>` the object are trivial, as shown in later chapters).
...
:term:`vertices <Vertex>` are defined.


WorldSpace
^^^^^^^^^^

WorldSpace is a top-level space, independent of :term:`NDC <Normalized Device Coordinates>`, that we choose to use.  It is arbitrary.
If you were to model a racetrack for a racing game, the :term:`origin <Origin>` of WorldSpace may be the center of that racetrack.
...
For our demo with paddles, the author arbitrarily defines the :term:`WorldSpace <World Space>` to be 20 units wide, 20 units tall, with the origin at the center.
...
The author prefers to view :term:`transformations <Transformation>` as changes to the graph paper,
as compared to view transformations as changes to points.

As such, for placing paddle1, we can view the :term:`translation <Translation>` as a change
to the graph paper relative to :term:`world space <World Space>` coordinates (only incidentally
...
Although we will think of the paddle's :term:`vertices <Vertex>` as relative to its own space
...
number of units in the y direction, 0 during the first :term:`frame <Frame>`, based off of user input.  The :term:`origin <Origin>`
is translated, and the graph paper comes with it, onto which you can
plot the vertices.  Notice that the :term:`coordinate system <Coordinate System>` labels below the plot and to the left of the plot
is unchanged.  That is :term:`world space <World Space>`, which has not changed.
...
Now that the :term:`transformation <Transformation>` has happened, the vertices are all in :term:`world space <World Space>`.
...
was the same as world space, has changed, bringing the :term:`vertices <Vertex>` along with it.

Same goes for Paddle 2's :term:`modelspace<Modelspace>`, relative to its :term:`translation <Translation>`, which
...
Our paddles are now well outside of :term:`NDC <Normalized Device Coordinates>`, and as such, they would not
be displayed, as they would be :term:`clipped <Clipping>` out.  Their values are outside
of -1.0 to 1.0.  All we will need to do to convert them from
:term:`world space <World Space>` to NDC is divide each component, x and y, by 10.

As a demonstration of how :term:`scaling <Scaling>` works, let's make an
...
by "scale"ing each component of the :term:`vertices <Vertex>` by some coefficient.
...
are the offset from the world space's :term:`origin <Origin>` to the center of the paddle, i.e. the :term:`translation <Translation>`.
...
named "mathutils.py".  The main class in this module is "Vector",
which has two components: and x value, and a y value.  To add a vector
to another one on the right hand side of the '+' symbol, we just add
the respective components together, and create a new Vector.



``Vector`` is the same data structure introduced earlier; it is unchanged here.

In a Python class, we can overload the '+' symbol, to make
objects addable, by implementing the :py:meth:`__add__ <modelviewprojection.mathutils.MultiVectorBase.__add__>`
method.  The definition is just saying pairwise add each component.
Don't stress about understanding the implementation, look at the
tests listed after.

.. literalinclude:: _gacalc_src/g2.py
   :language: python
   :start-after: doc-region-begin Vector __add__ method
   :end-before: doc-region-end Vector __add__ method
   :linenos:
   :lineno-match:
   :caption: gacalc/g2.py


We can also model the opposite procedure, subtraction,
by implementing the :py:meth:`__sub__ <modelviewprojection.mathutils.MultiVectorBase.__sub__>`
method.

.. literalinclude:: _gacalc_src/g2.py
   :language: python
   :start-after: doc-region-begin Vector __sub__ method
   :end-before: doc-region-end Vector __sub__ method
   :linenos:
   :lineno-match:
   :caption: gacalc/g2.py
...
things to note, :py:meth:`translate <modelviewprojection.mathutils.translate>`
is a function on the mathutils module, not a method
on Vector class, and it's wrapped in a class named
:class:`InvertibleFunction <modelviewprojection.mathutils.InvertibleFunction>`.
...
but the function for translating, named "f", and the inverse of "f" named "f_inv", take a Vector.
This is because we will be translating many Vectors using the same amount.


:term:`Invertible functions <Invertible Function>` are stored in pairs, with the "active" function
being the first one passed to the constructor.  So for translate above,
the adding of the Vectors will be the function, but
:class:`InvertibleFunction <modelviewprojection.mathutils.InvertibleFunction>`
holds onto the second function, for later use to be able to undo the function's
application.

Here is how the ``translate`` function is used.
...
by that amount.  "translate" returns an
:class:`InvertibleFunction <modelviewprojection.mathutils.InvertibleFunction>`, which is
Callable[Vector, Vector].  Callable[Vector, Vector] is a type which
is a function that takes a Vector as input, and returns a Vector (in this case,
the output is the input, translated by Vector(2.0, 3.0).

On the next 3 lines, we call the function t, passing in a Vector to be translated, and
...
Here's how :class:`InvertibleFunction <modelviewprojection.mathutils.InvertibleFunction>` is implemented:

.. literalinclude:: _gacalc_src/functions.py
   :language: python
   :start-after: doc-region-begin invertible function class signature
   :end-before: doc-region-end invertible function class signature
   :linenos:
   :lineno-match:
   :caption: gacalc/functions.py


.. literalinclude:: _gacalc_src/functions.py
   :language: python
   :start-after: doc-region-begin invertible function members
   :end-before: doc-region-end invertible function members
   :linenos:
   :lineno-match:
   :caption: gacalc/functions.py
...
:py:meth:`__call__ <modelviewprojection.mathutils.InvertibleFunction.__call__>`
method.

.. literalinclude:: _gacalc_src/functions.py
   :language: python
   :start-after: doc-region-begin composable function call signature
   :end-before: doc-region-end composable function call signature
   :linenos:
   :lineno-match:
   :caption: gacalc/functions.py
...
.. literalinclude:: _gacalc_src/functions.py
   :language: python
   :start-after: doc-region-begin composable function call body
   :end-before: doc-region-end composable function call body
   :linenos:
   :lineno-match:
   :caption: gacalc/functions.py

We can get the inverse of the procedure by the following
:py:meth:`inverse <modelviewprojection.mathutils.inverse>`, procedure

.. literalinclude:: _gacalc_src/functions.py
   :language: python
   :start-after: doc-region-begin inverse signature
   :end-before: doc-region-end inverse signature
   :linenos:
   :lineno-match:
   :caption: gacalc/functions.py
...
.. literalinclude:: _gacalc_src/functions.py
   :language: python
   :start-after: doc-region-begin inverse body
   :end-before: doc-region-end inverse body
   :linenos:
   :lineno-match:
   :caption: gacalc/functions.py
...
Back to method's on the Vector class.  We can also define scaling of a Vector,
by implementing multiplication of Vector's by a scalar, meaning a real number
that scaled the Vector by the same amount in all directions.  We do this
by implementing the :py:meth:`__mul__ <modelviewprojection.mathutils.MultiVectorBase.__mul__>`
and :py:meth:`__rmul__ <modelviewprojection.mathutils.MultiVectorBase.__rmul__>` methods, where __rmul__ just means
that this object is on the right hand side of the multiplication.

.. literalinclude:: _gacalc_src/g2.py
   :language: python
   :start-after: doc-region-begin Vector __mul__ method
   :end-before: doc-region-end Vector __mul__ method
   :linenos:
   :lineno-match:
   :caption: gacalc/g2.py
...
:py:meth:`uniform_scale <modelviewprojection.mathutils.uniform_scale>`,
but the function for scaling "f", and the inverse of "f" named "f_inv", take a Vector.
This is because we will be scaling many Vectors using the same scaling factor.

.. literalinclude:: _gacalc_src/transforms.py
   :language: python
   :start-after: doc-region-begin uniform scale signature
   :end-before: doc-region-end uniform scale body
   :linenos:
   :lineno-match:
   :caption: gacalc/transforms.py
...
.. literalinclude:: ../../src/modelviewprojection/demos/demo06.py
   :language: python
   :start-after: doc-region-begin instantiate paddles
   :end-before: doc-region-end instantiate paddles
   :linenos:
   :lineno-match:
   :caption: src/modelviewprojection/demos/demo06.py
...
The :term:`coordinate system <Coordinate System>` now resets back to the coordinate system specified
on the left and below.
Now, we must scale everything by 1/10.  I have
not included a picture of that here.  :term:`Scaling` happens relative to
the :term:`origin <Origin>`, so the picture would be the same, just with different labels
...
The :term:`coordinate system <Coordinate System>` is reset.  Now scale everything by 1/10.  I have
not included a picture of that here.  :term:`Scaling` happens relative to
the :term:`origin <Origin>`, so the picture would be the same, just with different labels
on the bottom and on the left.
