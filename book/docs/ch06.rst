...
named "mathutils.py".  The main class in this module is "Vector2",
...
the respective components together, and create a new Vector2.



``Vector2`` is the same data structure introduced earlier; it is unchanged here.
...
on Vector2 class, and it's wrapped in a class named
...
but the function for translating, named "f", and the inverse of "f" named "f_inv", take a Vector2.
This is because we will be translating many Vector2s using the same amount.
...
the adding of the Vector2s will be the function, but
:class:`InvertibleFunction <modelviewprojection.mathutils.InvertibleFunction>`
...
Callable[Vector2, Vector2].  Callable[Vector2, Vector2] is a type which
is a function that takes a Vector2 as input, and returns a Vector2 (in this case,
the output is the input, translated by Vector2(x=2.0, y=3.0).

On the next 3 lines, we call the function t, passing in a Vector2 to be translated, and
...
Back to method's on the Vector2 class.  We can also define scaling of a Vector2,
by implementing multiplication of Vector2's by a scalar, meaning a real number
that scaled the Vector2 by the same amount in all directions.  We do this
...
but the function for scaling "f", and the inverse of "f" named "f_inv", take a Vector2.
This is because we will be scaling many Vector2s using the same scaling factor.
