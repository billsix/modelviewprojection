...
Load src/modelviewprojection/demos/demo04.py in Spyder and hit the play button.
...
And while we are at it, let's go ahead and create data structures for a :term:`Vector <Vector (Vector2 / Vector3)>`,
and for the collection of :term:`vertices <Vertex>` that make up a Paddle.
...
.. literalinclude:: ../../src/modelviewprojection/demos/demo04.py
   :language: python
   :start-after: doc-region-begin define vector class
   :end-before: doc-region-end define vector class
   :linenos:
   :lineno-match:
   :caption: src/modelviewprojection/demos/demo04.py
...
- If the user presses 's' this :term:`frame <Frame>`, subtract 0.1 from the y component of each of
...
:term:`Monitors <Monitor>` can have variable :term:`frame-rates <Frame Rate>`, and in order to ensure
that movement is consistent across different monitors, we choose
to only flush the screen at 60 :term:`hertz <Hertz>` (frames per second).
...
* We're still near the beginning of the :term:`event loop <Event Loop>`, and we
