...
Remove repetition in the coordinate :term:`transformations <Transformation>`,
as previous demos had very similar transformations,
especially from :term:`camera space <Camera Space>` to :term:`NDC <Normalized Device Coordinates>` space.
Each edge of the graph of objects
should only be specified once per :term:`frame <Frame>`.
...
Noticing in the previous demos that the lower parts of the :term:`transformations <Transformation>`
...
get :term:`NDC <Normalized Device Coordinates>` data,
...
.. literalinclude:: ../../src/modelviewprojection/demos/demo14.py
   ...
   :linenos:
   :lineno-match:
   :caption: src/modelviewprojection/demos/demo14.py
...
objects, it is unwise to put this much repetition in the :term:`transformation <Transformation>`
sequence.  Especially if we might change how the camera operates,
or from :term:`perspective <Perspective Projection>` to :term:`ortho <Orthographic Projection>`.  It would required a lot of code changes.
And I don't like reading from the bottom of the code up.  Code doesn't
execute that way.  I want to read from top to bottom.

When reading the :term:`transformation <Transformation>` sequences in the previous demos from top down
...
With a :term:`function stack <Function Stack>`, the function at the top of the stack (f5) is applied first,
...
This means that from :term:`modelspace<Modelspace>` to :term:`world space <World Space>`, we can now
...
Load src/modelviewprojection/demos/demo16.py in Spyder and hit the play button.
...
:term:`Function stack <Function Stack>`.  Internally it has a list, where index 0 is
the bottom of the stack.  In python we can store any object
as a variable, and we will be storing functions which transform
a :term:`vector <Vector (Vector2 / Vector3)>` to another vector, through the "modelspace_to_ndc_fn" method.



.. literalinclude:: ../../src/modelviewprojection/mathutils.py
   :start-after: doc-region-begin define function stack class
   :end-before: doc-region-end function stack class signature
   :language: python
   :linenos:
   :lineno-match:
...
.. literalinclude:: ../../src/modelviewprojection/mathutils.py
   :start-after: doc-region-begin function stack class fields
   :end-before: doc-region-end function stack push signature
   :language: python
   :linenos:
   :lineno-match:
   :caption: src/modelviewprojection/mathutils.py
...
.. literalinclude:: ../../src/modelviewprojection/mathutils.py
   :start-after: doc-region-begin function stack push body
   :end-before: doc-region-end function stack pop signature
   :language: python
   :linenos:
   :lineno-match:
   :caption: src/modelviewprojection/mathutils.py
...
.. literalinclude:: ../../src/modelviewprojection/mathutils.py
   :start-after: doc-region-begin function stack pop body
   :end-before: doc-region-end function stack clear signature
   :language: python
   :linenos:
   :lineno-match:
   :caption: src/modelviewprojection/mathutils.py
...
.. literalinclude:: ../../src/modelviewprojection/mathutils.py
   :start-after: doc-region-begin function stack clear body
   :end-before: doc-region-end function stack compose signature
   :language: python
   :linenos:
   :lineno-match:
   :caption: src/modelviewprojection/mathutils.py
...
.. literalinclude:: ../../src/modelviewprojection/mathutils.py
   :start-after: doc-region-begin function stack compose body
...
Push :term:`identity <Identity>` onto the stack, which will will never pop off of the stack.


.. literalinclude:: ../../tests/test_mathutils.py
   :language: python
   ...
   :linenos:
   :lineno-match:
   :caption: tests/test_mathutils.py
...
Event Loop
^^^^^^^^^^
.. literalinclude:: ../../src/modelviewprojection/demos/demo16.py
   :language: python
   :start-after: doc-region-begin begin event loop
   :end-before: doc-region-end begin event loop
   :linenos:
   :lineno-match:
   :caption: src/modelviewprojection/demos/demo16.py
...
In previous demos, cs_to_ndc_space_fn was always
the last function called in the method chained pipeline.  Put it on the bottom of the stack,
by pushing it first,
so that "modelspace_to_ndc_fn" calls this function last.  Each subsequent push will add
...
from :term:`modelspace<Modelspace>` to :term:`world space <World Space>` backwards; this time because the transformations
...
to :term:`world space <World Space>` like so, read top to bottom:
...
But, since we need to transform :term:`world-space <World Space>` to :term:`camera space <Camera Space>`,
...
Unlike in previous demos in which we read the transformations
from :term:`modelspace<Modelspace>` to :term:`world space <World Space>` backwards; because the transformations
are on a stack, the fns on the model stack can
be read forwards, where each operation translates/rotates/scales
the current space
...
this results in coordinate data in :term:`NDC <Normalized Device Coordinates>` space,
...
:term:`lambda stack <Function Stack>` will convert from world space to :term:`NDC <Normalized Device Coordinates>`.
This will allow us to just add the transformations from
:term:`world space <World Space>` to paddle2 space on the stack.
...
remove all fns from the :term:`function stack <Function Stack>`, as the next frame will set them
...
:term:`Swap buffers <Swap Buffers>` and execute another iteration of the :term:`event loop <Event Loop>`
...
"modelspace_to_ndc_fn" on the :term:`modelspace<Modelspace>` vector.  In Demo 19, we will be using
OpenGL 2.1 :term:`matrix stacks <Matrix Stack>`.  Although we don't have the code for the OpenGL
...
to :term:`NDC <Normalized Device Coordinates>` transformations from the ModelView and Projection matrix stacks.
