...
Make a moving camera in 3D space.  Use Ortho to transform a rectangular prism, defined relative to :term:`camera space <Camera Space>`,
into :term:`NDC <Normalized Device Coordinates>`.
...
This is because it gets ":term:`clipped <Clipping>` out", as the geometry will be outside
of :term:`NDC <Normalized Device Coordinates>`, (-1 to 1 on all three axis).  We could fix this by making
...
from the viewer would appear smaller.  This will be fixed in demo18.
...
Load src/modelviewprojection/demos/demo17.py in Spyder and hit the play button.
...
Before starting this demo, run src/modelviewprojection/mvpvisualization/modelvieworthoprojection.py, as
...
is placed and oriented in :term:`world space <World Space>`, use the buttons to change the camera's position
...
#. :term:`translate <Translation>` to the camera's position, using the actual position values
   of camera position in world space coordinates.
#. rotate around the local y axis
#. rotate around the local x axis

To visualize this, run "python src/modelviewprojection/mvpvisualization/modelvieworthoprojection.py"

The ordering of 1) before 2) and 3) should be clear, as
we are imagining a :term:`coordinate system <Coordinate System>` that moves, just
like we do for the :term:`modelspace<Modelspace>` to world space transformations.
The ordering of 2) before 3) is very important, as two :term:`rotations <Rotation>`
around different axes are :term:`not commutative <Non-commutativity of Rotations>`, meaning that you can't
...
space by making a moving coordinate system (composed of an :term:`origin <Origin>`, 1 unit
...
orientation of :term:`NDC <Normalized Device Coordinates>`.  Looking at the following graph,
...
and then to :term:`camera space <Camera Space>` (which is going in the opposite direction of the arrow, therefore requires
an :term:`inverse <Inverse>` operation, because to plot data we go from :term:`modelspace<Modelspace>` to :term:`screen space <Screen Space>` on the graph.

Given that the inverse of a sequence of :term:`transformations <Transformation>` is the sequence backwards,
...
..
   ...
   Take a look at src/modelviewprojection/mvpvisualization/modelviewperspectiveprojection.py to see an animated
   ...
   Take a look at src/modelviewprojection/mvpvisualization/modelviewperspectiveprojection.py to see an
...
.. literalinclude:: ../../src/modelviewprojection/demos/demo17.py
   :language: python
   :start-after: doc-region-begin define camera class
   :end-before: doc-region-end define camera class
   :linenos:
   :lineno-match:
   :caption: src/modelviewprojection/demos/demo17.py
...
and backwards.  Unlike the :term:`camera space <Camera Space>` to :term:`world space <World Space>`
...
.. literalinclude:: ../../src/modelviewprojection/mathutils.py
   ...
   :end-before: doc-region-end ortho signature
   :language: python
   :linenos:
   :lineno-match:
...
.. literalinclude:: ../../src/modelviewprojection/mathutils.py
   :start-after: doc-region-begin ortho body
...
We will make a wrapper function cs_to_ndc_space_fn which
calls ortho, setting the size of the rectangular prism.

.. literalinclude:: ../../src/modelviewprojection/mathutils.py
   :start-after: doc-region-begin define camera space to ndc
   :end-before: doc-region-end camera space to ndc signature
   :language: python
   :linenos:
   :lineno-match:
   :caption: src/modelviewprojection/mathutils.py
...
.. literalinclude:: ../../src/modelviewprojection/mathutils.py
   :start-after: doc-region-begin camera space to ndc body
