...
With the depth test enabled, the square is now correctly hidden behind
...
over it, because without a depth buffer the object drawn last clobbers the
color of whatever was already at that pixel.  Now each fragment also carries
its depth, and ``GL_GREATER`` keeps a newly drawn fragment only when it is
nearer than the value already in the depth buffer, so the farther square no
