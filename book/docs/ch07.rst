...
Learn about :term:`rotations <Rotation>`.  This demo does not
work correctly, because of a misunderstanding
of how to interpret a sequence of :term:`transformations <Transformation>`.
...
Load src/modelviewprojection/demos/demo07.py in Spyder and hit the play button.
...
Besides :term:`translate <Translation>` and :term:`scale <Scaling>`, the third main operation in computer
graphics is to :term:`rotate <Rotation>` an object.
...
all of the object's :term:`vertices <Vertex>` around (0,0).
...
us to rotate all vertices, wherever they are, around the :term:`origin <Origin>` (0,0), by some
angle :inlinetex:`\theta`.
...
of (x,y) to be ":inlinetex:`\beta`".  As a reminder, we want to rotate by a different
angle, called ":inlinetex:`\theta`".
...
Before we can rotate by ":inlinetex:`\theta`", first we need to be able to rotate by
90 degrees, or :inlinetex:`\pi`/2.  So to rotate (cos(:inlinetex:`\beta`), sin(:inlinetex:`\beta`)) by :inlinetex:`\pi`/2,
we get (cos(:inlinetex:`\beta` + :inlinetex:`\pi`/2), sin(:inlinetex:`\beta` + :inlinetex:`\pi`/2)).
...
Now forget about ":inlinetex:`\beta`", and remember that our goal is to rotate by angle
":inlinetex:`\theta`".  Look at the picture below, while turning your head slightly to the
left.  x' and y' look just like our normal Cartesian plane and unit circle,
combined with the ":inlinetex:`\theta`"; it looks like what we already know from high
school geometry.
...
So with this new frame of reference, we can rotate x' by ":inlinetex:`\theta`",
...
.. inlinetex::
    ...
    $ \vec{r}(\vec{a}; \theta) =  cos(\theta)*\vec{x'} + sin(\theta)*\vec{y'} $
...
A problem we have now is how to calculate cosine and sine of :inlinetex:`\beta` + :inlinetex:`\pi`/2,
because we haven't actually calculated :inlinetex:`\beta`; we've calculated sine
and cosine of :inlinetex:`\beta` by dividing the x value by the magnitude of the a, and the sine of :inlinetex:`\beta`
...
.. TODO (2026-07-09, tasks/book-rotate-prose-update.md): the included
   code changed -- ``rotate`` is now bound directly from gacalc's
   ``plane_rotation`` factory (the plane e1^e2 is derived once at module
   import; each call is just the half-angle rotor).  The prose around this
   include still narrates the old build-a-to-vector-and-call-rotor_rotation
   shape and needs rewriting to the plane+angle story.

.. literalinclude:: ../../src/modelviewprojection/mathutils.py
   :language: python
   :start-after: doc-region-begin define rotate 2d
   :end-before: doc-region-end define rotate 2d
   :linenos:
   :lineno-match:
...
.. literalinclude:: ../../src/modelviewprojection/demos/demo07.py
   :language: python
   :start-after: doc-region-begin define paddle class
   :end-before: doc-region-end define paddle class
   :linenos:
   :lineno-match:
   :caption: src/modelviewprojection/demos/demo07.py
...
we should :term:`translate <Translation>` to its position, and then rotate
...
:term:`translate <Translation>`, :term:`scale <Scaling>`, and :term:`rotate <Rotation>` are all defined correctly.  The paddles
are not rotating around their center.

Let's take a look in detail about what our paddle-space to
:term:`world space <World Space>` :term:`transformations <Transformation>` are doing.
...
* :term:`modelspace<Modelspace>` :term:`vertices <Vertex>`
...
* Reset the :term:`coordinate system <Coordinate System>`
