...
shown so far.  The nodes represent a coordinate system (i.e. origin and axises),
...
A definition of this function is not provided in this text, because we can't change it.
...
.. math::
   \begin{equation*}
    f_{nickel}^{penny}(x) = 5.0 * x \\
  \end{equation*}

.. math::
   \begin{equation*}
    f_{dime}^{penny}(x) = 10.0 * x \\
  \end{equation*}


.. math::
   \begin{equation*}
    f_{quarter}^{penny}(x) = 25.0 * x \\
  \end{equation*}


.. math::
   \begin{equation*}
    f_{dollar}^{penny}(x) = 100.0 * x
  \end{equation*}
...
direction of the edge, we must apply the inverse
of that function.  These functions are invertible by
taking the reciprical of the coefficient, thus satisfying
...
.. math::
    ...
    \begin{equation*}
    \begin{split}
    f_{penny}^{quarter}(x) & = {f_{penny}^{quarter}}^{-1}(x) \\
    & =  1/25.0 * x
    \end{split}
    \end{equation*}
...
.. math::
    ...
    \begin{equation}
    \begin{split}
    {f_{penny}^{quarters}}^{-1}(100)  & = 1/25.0 * x\\
                     & = 4
    \end{split}
    \end{equation}
...
Notice in the last equation that we defined the function via function composition,
and didn't specify and arguments.  We just focus on the types of units in and the units
...
D to space B (defined below)?  What do those names mean?  It doesn't matter.  All that
matters is that the function exists and that the function is invertible.
