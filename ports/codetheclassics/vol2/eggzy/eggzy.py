...
# All rights in the original code and assets remain with Raspberry Pi Press and
# the authors; this port is included for educational use.  The game logic below
# is unchanged from the original -- only the framework layer differs: it runs on
# `pgzero_gl`, a shim reimplementing the slice of PyGame Zero this game uses on
# GLFW + OpenGL 3.3 core (the renderer the ModelViewProjection book ends on).
