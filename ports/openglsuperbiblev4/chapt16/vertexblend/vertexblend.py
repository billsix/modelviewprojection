...
camera_pos: list[float] = [50.0, 75.0, 50.0, 1.0]
camera_zoom: float = 0.4
light_pos: np.ndarray = np.array([-50.0, 100.0, 50.0, 1.0], dtype=np.float32)
...
def prepare_shader() -> None:
    ...
    fname: str = os.path.join(PWD, "shaders", "skinning.vs")
...
def draw_cylinder(
    ...
    num_facets: int = 50
    num_sections: int = 50
    angle_incr: float = (2.0 * math.pi) / num_facets

    if start_weight == 0.5:
        influence_start: float = sphere_of_influence
    else:
        influence_start = 1.0 - sphere_of_influence

    for i in range(num_facets):
        a1: float = i * angle_incr
        a2: float = ((i + 1) % num_facets) * angle_incr
        y1f: float = math.sin(a1) * diameter1
        y1o: float = math.sin(a1) * diameter2
        z1f: float = math.cos(a1) * diameter1
        z1o: float = math.cos(a1) * diameter2
        y2f: float = math.sin(a2) * diameter1
        y2o: float = math.sin(a2) * diameter2
        z2f: float = math.cos(a2) * diameter1
        z2o: float = math.cos(a2) * diameter2
        n1y, n1z = y1f, z1f
        n2y, n2z = y2f, z2f
        ...
        for j in range(num_sections):
            param_end: float = float(j + 1) / float(num_sections)
            length_end: float = param_end * length
            y1e: float = y1f + (param_end * (y1o - y1f))
            z1e: float = z1f + (param_end * (z1o - z1f))
            y2e: float = y2f + (param_end * (y2o - y2f))
            z2e: float = z2f + (param_end * (z2o - z2f))

            if not use_blending:
                w_end: float = 1.0
            elif influence_start > 0.0 and param_end <= influence_start:
                p: float = param_end * (1.0 / influence_start)
...
def invert_matrix(m: np.ndarray) -> np.ndarray:
    ...
    m44: np.ndarray = m.reshape((4, 4), order="F")
    inv44: np.ndarray = np.linalg.inv(m44)
...
def draw_models() -> None:
    ...
    mv2_it_4: np.ndarray = invert_matrix(mv2)
    mv2_it_3: np.ndarray = np.zeros(9, dtype=np.float32)
    for i in range(9):
